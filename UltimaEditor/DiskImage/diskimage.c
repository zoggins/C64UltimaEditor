...
int di_status(DiskImage *di, char *status, size_t statusLen) {
  ...
  if (di->status == 254) {
    switch (di->type) {
    case D64:
      sprintf_s(status, statusLen, "73,cbm dos v2.6 1541,00,00");
      break;
    case D71:
      sprintf_s(status, statusLen, "73,cbm dos v3.0 1571,00,00");
      break;
    case D81:
      sprintf_s(status, statusLen, "73,copyright cbm dos v10 1581,00,00");
      break;
    }
    return 73;
  }
  ...
  while (err->number >= 0) {
    if (di->status == err->number) {
      sprintf_s(status, statusLen, "%02d,%s,%02d,%02d", di->status, err->string, di->statusts.track, di->statusts.sector);
      return di->status;
    }
    ++err;
  }
  sprintf_s(status, statusLen, "%02d,unknown error,%02d,%02d", di->status, di->statusts.track, di->statusts.sector);
  return di->status;
}


int set_status(DiskImage *di, int status, unsigned char track, unsigned char sector) {
  di->status = status;
...
unsigned char interleave(ImageType type) {
  switch (type) {
  case D64:
    return 10;
    break;
...
unsigned char di_sectors_per_track(ImageType type, unsigned char track) {
  switch (type) {
  case D71:
    if (track > 35) {
      track -= 35;
...
TrackSector alloc_next_ts(DiskImage *di, TrackSector prevts) {
  unsigned char *bam;
  unsigned char spt, s1, s2, t1, t2, bpt, boff, res1, res2;
  TrackSector ts;
...
int di_rename(DiskImage *di, unsigned char *oldrawname, unsigned char *newrawname, FileType type) {
  ...
  rde = find_file_entry(di, oldrawname, type);
  if (rde) {
